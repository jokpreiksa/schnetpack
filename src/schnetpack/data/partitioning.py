...
def split_list_with_indexes(lst, n):
    result_lists = []
    result_indexes = []
    len_list = len(lst)
    for i in range(0, len(lst), n):
        if i + n < len_list:
            result_indexes.append(list(range(i, i + n)))
        else:
            new_n = len_list - i
            result_indexes.append(list(range(i, i + new_n)))
...
def train_test_split2(
    ...
    """
    Function works similarly to train_test_split however this version enables user to split files by taking n entries
    in the row to the same split, which is useful if data set has n conformers of the same compound. (Made for QMugs DB)
    ...
    Args:
        ...
        split_file (str): Path to split file. If file exists, splits will
                          be loaded. Otherwise, a new file will be created
                          where the generated split is stored.
        nth (int): splits the data keeping n consecutive elements in the same list. (useful
        for data sets with number(nth) of different conformers)
    Returns:
        spk.data.AtomsDataSubset: subset with training data
        spk.data.AtomsDataSubset: subset with validation data
        spk.data.AtomsDataSubset: subset with test data
    ...
    if nth is not None or nth == 1:
        orig_data = data
        # print(len(orig_data))
        data = split_list_with_indexes(data, nth)
        # print(len(orig_data), len(data))
    ...
    if nth is not None or nth != 1:
        # print(train_idx)
        orig_train_idx = [idx for i in train_idx for idx in data[i]]
        print('Train:', len(orig_data), max(orig_train_idx))
        train = create_subset(orig_data, orig_train_idx)

        orig_val_idx = [idx for i in val_idx for idx in data[i]]
        print('Val:', len(orig_data), max(orig_val_idx))
        val = create_subset(orig_data, orig_val_idx)

        orig_test_idx = [idx for i in test_idx for idx in data[i]]
        print('Test:', len(orig_data), max(orig_test_idx))
        test = create_subset(orig_data, orig_test_idx)
